# Yolo Image Classification Implementation
...
'''
The show detected images function iterates through bounding boxes and draws the physical boxes with the label
...
def show_detected_images(img, bounding_box_ids, all_bounding_boxes, class_ids, confidence_values, width_ratio,
    ...
    for index in bounding_box_ids:
        ...
        # we are not going to detect every objects just PERSON, CAR, and BUS
        ...
        if class_ids[index[0]] == 5:
            cv2.rectangle(img, (x, y), (x + w, y + h), (0, 0, 255), 2)
            class_with_confidence = 'BUS' + str(int(confidence_values[index[0]] * 100)) + '%'
            cv2.putText(img, class_with_confidence, (x, y - 10), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.0, (0, 0, 255), 1)
